...
#Shufflev2Net
def channel_shullfe(x,group):
    batch_size, channles, height, width = x.size()
    channles_pre_group = channles //group #均分通道，获取每个组对应的通道
    x = x.view(batch_size, group, channles_pre_group, height, width)
...
class ShuffleNetV2_InvertedResidual(nn.Module):
    def __init__(self, input_c, output_c, stride):
        ...
        #如果stride == 1,输入特征图的通道数为输出特征图的2倍
        assert (self.stride != 1) or (input_c == branch_features * 2)
        #步长为 2 ，下采样模块，左分支第二个1*1卷积调整通道数，右分支第一个1*1卷积调整通道数
        if self.stride == 2:
            ...
            self.branch2 = nn.Sequential(
                ...
                nn.BatchNorm2d(branch_features),
                nn.ReLU(inplace=True),
                self.depthwise_conv(branch_features, branch_features, kernel_s=3, stride=self.stride, padding=1, bias=False),
                nn.BatchNorm2d(branch_features),
    ...
    def depthwise_conv(self, input_c, output_c, kernel_s, stride=1, padding=0, bias=False):
...
class SPPFCSPC(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.m(x1)
        x3 = self.m9(x2)
        y1 = self.cv6(self.cv5(torch.cat((x1,x2,x3, self.m13(x3)),1)))
        y2 = self.cv2(x)
        return self.cv7(torch.cat((y1, y2), dim=1))
...
class CARAFE(nn.Module):
    ...
    def forward(self, X):
        ...
        W = self.comp(X)  # b * m * h * w  降维
        W = self.enc(W)  # b * 100 * h * w
        W = self.pix_shf(W)  # b * 25 * h_ * w_   特征图改变大小   h/scale/scale h*scale w*scale
        W = torch.softmax(W, dim=1)  # b * 25 * h_ * w_    通道上面进行softmax

        X = self.upsmp(X)  # b * c * h_ * w_     进行上采样
        X = self.unfold(X)  # b * 25c * h_ * w_   进行感受野分割      channle*kernel size
